...
namespace Microsoft.Data.SqlClient
{
    ...
    internal sealed partial class TdsParser
    {
        ...
        internal Task WriteSqlVariantDataRowValue<T>(T value, TdsParserStateObject stateObj, bool canAccumulate = true)
        {
            ...
            {
                length = GetEncodingCharLength(ValueTypeConverter<T, string>.Convert(value), length, 0, _defaultEncoding);
            }
            ...
            switch (metatype.TDSType)
            {
                case TdsEnums.SQLFLT4:
                    ...
                    WriteFloat(ValueTypeConverter<T, float>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLFLT8:
                    WriteSqlVariantHeader(10, metatype.TDSType, metatype.PropBytes, stateObj);
                    WriteDouble(ValueTypeConverter<T, double>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLINT8:
                    WriteSqlVariantHeader(10, metatype.TDSType, metatype.PropBytes, stateObj);
                    WriteLong(ValueTypeConverter<T, long>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLINT4:
                    WriteSqlVariantHeader(6, metatype.TDSType, metatype.PropBytes, stateObj);
                    WriteInt(ValueTypeConverter<T, int>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLINT2:
                    WriteSqlVariantHeader(4, metatype.TDSType, metatype.PropBytes, stateObj);
                    WriteShort(ValueTypeConverter<T, short>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLINT1:
                    WriteSqlVariantHeader(3, metatype.TDSType, metatype.PropBytes, stateObj);
                    stateObj.WriteByte(ValueTypeConverter<T, byte>.Convert(value));
                    break;

                case TdsEnums.SQLBIT:
                    WriteSqlVariantHeader(3, metatype.TDSType, metatype.PropBytes, stateObj);
                    if (ValueTypeConverter<T, bool>.Convert(value))
                        stateObj.WriteByte(1);
                    else
                        stateObj.WriteByte(0);
                ...
                case TdsEnums.SQLBIGVARBINARY:
                    {
                        byte[] b = ValueTypeConverter<T, byte[]>.Convert(value);
                ...
                case TdsEnums.SQLBIGVARCHAR:
                    {
                        string s = ValueTypeConverter<T, string>.Convert(value);
                ...
                case TdsEnums.SQLUNIQUEID:
                    {
                        Guid guid = ValueTypeConverter<T, Guid>.Convert(value);
                        Span<byte> b = stackalloc byte[16];
                ...
                case TdsEnums.SQLDATETIME:
                    {
                        TdsDateTime dt = MetaType.FromDateTime(ValueTypeConverter<T, DateTime>.Convert(value), 8);
                ...
                case TdsEnums.SQLMONEY:
                    {
                        WriteSqlVariantHeader(10, metatype.TDSType, metatype.PropBytes, stateObj);
                        WriteCurrency(ValueTypeConverter<T, decimal>.Convert(value), 8, stateObj);
                        break;
                    }
                ...
                case TdsEnums.SQLNUMERICN:
                    {
                        ...
                        var decValue = ValueTypeConverter<T, decimal>.Convert(value);
                ...
                case TdsEnums.SQLTIME:
                    ...
                    WriteTime(ValueTypeConverter<T, TimeSpan>.Convert(value), metatype.Scale, 5, stateObj);
                    break;

                case TdsEnums.SQLDATETIMEOFFSET:
                    WriteSqlVariantHeader(13, metatype.TDSType, metatype.PropBytes, stateObj);
                    stateObj.WriteByte(metatype.Scale); //propbytes: scale
                    WriteDateTimeOffset(ValueTypeConverter<T, DateTimeOffset>.Convert(value), metatype.Scale, 10, stateObj);
                    break;
            ...
            switch (metadata.baseTI.metaType.NullableType)
            {
                ...
                case TdsEnums.SQLIMAGE:
                    ...
                    actualLengthInBytes = (isSqlType)
                        ? ValueTypeConverter<T, SqlBinary>.Convert(value).Length
                        : ValueTypeConverter<T, byte[]>.Convert(value).Length;
                ...
                case TdsEnums.SQLTEXT:
                    ...
                    string stringValue = (isSqlType)
                        ? ValueTypeConverter<T, SqlString>.Convert(value).Value
                        : ValueTypeConverter<T, string>.Convert(value);
                ...
                case TdsEnums.SQLNTEXT:
                    actualLengthInBytes = (isSqlType
                        ? ValueTypeConverter<T, SqlString>.Convert(value).Value.Length
                        : ValueTypeConverter<T, string>.Convert(value).Length)
                        * 2;
        ...
        internal Task WriteBulkCopyValue<T>(T value, SqlMetaDataPriv metadata, TdsParserStateObject stateObj, bool isSqlType, bool isDataFeed, bool isNull)
        {
            ...
            try
            {
                ...
                if (!isDataFeed)
                {
                    switch (metatype.NullableType)
                    {
                        ...
                        case TdsEnums.SQLTEXT:
                            ...
                            if (isSqlType)
                            {
                                stringValue = ValueTypeConverter<T, SqlString>.Convert(value).Value;
                            }
                            else
                            {
                                stringValue = ValueTypeConverter<T, string>.Convert(value);
                            }
                ...
                else
                {
                    ...
                    internalWriteTask = stateObj.WriteByteArray(ValueTypeConverter<T, byte[]>.Convert(value), ccb, 0);
                }
            ...
            switch (type.NullableType)
            {
                case TdsEnums.SQLFLTN:
                    if (type.FixedLength == 4)
                        WriteFloat(ValueTypeConverter<T, SqlSingle>.Convert(value).Value, stateObj);
                    else
                    {
                        Debug.Assert(type.FixedLength == 8, "Invalid length for SqlDouble type!");
                        WriteDouble(ValueTypeConverter<T, SqlDouble>.Convert(value).Value, stateObj);
                    }
                ...
                case TdsEnums.SQLIMAGE:
                    {
                        ...
                        {
                            return stateObj.WriteByteArray(ValueTypeConverter<T, SqlBinary>.Convert(value).Value, actualLength, offset, canAccumulate: false);
                        }
                        else
                        {
                            Debug.Assert(value is SqlBytes);
                            return stateObj.WriteByteArray(ValueTypeConverter<T, SqlBytes>.Convert(value).Value, actualLength, offset, canAccumulate: false);
                        }
                    }
                ...
                case TdsEnums.SQLUNIQUEID:
                    {
                        ...
                        Span<byte> b = stackalloc byte[16];
                        SqlGuid sqlGuid = ValueTypeConverter<T, SqlGuid>.Convert(value);
                ...
                case TdsEnums.SQLBITN:
                    {
                        ...
                        if (ValueTypeConverter<T, SqlBoolean>.Convert(value).Value == true)
                            stateObj.WriteByte(1);
                        else
                            stateObj.WriteByte(0);
                ...
                case TdsEnums.SQLINTN:
                    if (type.FixedLength == 1)
                        stateObj.WriteByte(ValueTypeConverter<T, SqlByte>.Convert(value).Value);
                    else
                        if (type.FixedLength == 2)
                        WriteShort(ValueTypeConverter<T, SqlInt16>.Convert(value).Value, stateObj);
                    else
                            if (type.FixedLength == 4)
                        WriteInt(ValueTypeConverter<T, SqlInt32>.Convert(value).Value, stateObj);
                    else
                    {
                        Debug.Assert(type.FixedLength == 8, "invalid length for SqlIntN type:  " + type.FixedLength.ToString(CultureInfo.InvariantCulture));
                        WriteLong(ValueTypeConverter<T, SqlInt64>.Convert(value).Value, stateObj);
                    }
                ...
                case TdsEnums.SQLTEXT:
                    ...
                    {
                        string sch = new string(ValueTypeConverter<T, SqlChars>.Convert(value).Value);
                    ...
                    else
                    {
                        ...
                        return WriteEncodingChar(ValueTypeConverter<T, SqlString>.Convert(value).Value, actualLength, offset, _defaultEncoding, stateObj, canAccumulate: false);
                    }
                ...
                case TdsEnums.SQLXMLTYPE:
                    ...
                    if (value is SqlChars)
                    {
                        return WriteCharArray(ValueTypeConverter<T, SqlChars>.Convert(value).Value, actualLength, offset, stateObj, canAccumulate: false);
                    }
                    else
                    {
                        Debug.Assert(value is SqlString);
                        return WriteString(ValueTypeConverter<T, SqlString>.Convert(value).Value, actualLength, offset, stateObj, canAccumulate: false);
                    }

                case TdsEnums.SQLNUMERICN:
                    Debug.Assert(type.FixedLength <= 17, "Decimal length cannot be greater than 17 bytes");
                    WriteSqlDecimal(ValueTypeConverter<T, SqlDecimal>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLDATETIMN:
                    SqlDateTime dt = ValueTypeConverter<T, SqlDateTime>.Convert(value);
                ...
                case TdsEnums.SQLMONEYN:
                    {
                        WriteSqlMoney(ValueTypeConverter<T, SqlMoney>.Convert(value), type.FixedLength, stateObj);
                        break;
                    }
        ...
        private Task WriteUnterminatedValue<T>(T value, MetaType type, byte scale, int actualLength, int encodingByteSize, int offset, TdsParserStateObject stateObj, int paramSize, bool isDataFeed)
        {
            ...
            switch (type.NullableType)
            {
                ...
                case TdsEnums.SQLUDT:
                    {
                        ...
                        if (isDataFeed)
                        {
                            ...
                            return NullIfCompletedWriteTask(WriteStreamFeed(ValueTypeConverter<T, StreamDataFeed>.Convert(value), stateObj, paramSize));
                        }
                        else
                        {
                            if (type.IsPlp)
                            ...
                            return stateObj.WriteByteArray(ValueTypeConverter<T, byte[]>.Convert(value), actualLength, offset, canAccumulate: false);
                        }
                    }
                ...
                case TdsEnums.SQLUNIQUEID:
                    {
                        Debug.Assert(actualLength == 16, "Invalid length for guid type in com+ object");
                        Span<byte> b = stackalloc byte[16];
                        FillGuidBytes(ValueTypeConverter<T, Guid>.Convert(value), b);
                        stateObj.WriteByteSpan(b);
                        break;
                    }
                ...
                case TdsEnums.SQLBITN:
                    {
                        Debug.Assert(type.FixedLength == 1, "Invalid length for SqlBoolean type");
                        if (ValueTypeConverter<T, bool>.Convert(value) == true)
                            stateObj.WriteByte(1);
                        else
                            stateObj.WriteByte(0);
                ...
                case TdsEnums.SQLTEXT:
                    {
                        ...
                        if (isDataFeed)
                        {
                            ...
                            if (tdf == null)
                            {
                                return NullIfCompletedWriteTask(WriteXmlFeed(ValueTypeConverter<T, XmlDataFeed>.Convert(value), stateObj, needBom: true, encoding: _defaultEncoding, size: paramSize));
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            { // If LazyMat non-filled blob, send cookie rather than value
                                return stateObj.WriteByteArray(ValueTypeConverter<T, byte[]>.Convert(value), actualLength, 0, canAccumulate: false);
                            }
                            else
                            {
                                return WriteEncodingChar(ValueTypeConverter<T, string>.Convert(value), actualLength, offset, _defaultEncoding, stateObj, canAccumulate: false);
                            }
                        }
                    }
                case TdsEnums.SQLNCHAR:
                ...
                case TdsEnums.SQLXMLTYPE:
                    {
                        ...
                        if (isDataFeed)
                        {
                            ...
                            if (tdf == null)
                            {
                                return NullIfCompletedWriteTask(WriteXmlFeed(ValueTypeConverter<T, XmlDataFeed>.Convert(value), stateObj, IsBOMNeeded(type, value), Encoding.Unicode, paramSize));
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                // convert to cchars instead of cbytes
                                actualLength >>= 1;
                                return WriteString(ValueTypeConverter<T, string>.Convert(value), actualLength, offset, stateObj, canAccumulate: false);
                            }
                        }
                    }
                case TdsEnums.SQLNUMERICN:
                    Debug.Assert(type.FixedLength <= 17, "Decimal length cannot be greater than 17 bytes");
                    WriteDecimal(ValueTypeConverter<T, decimal>.Convert(value), stateObj);
                    break;

                case TdsEnums.SQLDATETIMN:
                    Debug.Assert(type.FixedLength <= 0xff, "Invalid Fixed Length");

                    TdsDateTime dt = MetaType.FromDateTime(ValueTypeConverter<T, DateTime>.Convert(value), (byte)type.FixedLength);
                ...
                case TdsEnums.SQLMONEYN:
                    {
                        WriteCurrency(ValueTypeConverter<T, decimal>.Convert(value), type.FixedLength, stateObj);
                        break;
                    }

                case TdsEnums.SQLDATE:
                    {
                        WriteDate(ValueTypeConverter<T, DateTime>.Convert(value), stateObj);
                        break;
                    }
                ...
                case TdsEnums.SQLTIME:
                    ...
                    WriteTime(ValueTypeConverter<T, TimeSpan>.Convert(value), scale, actualLength, stateObj);
                    break;
                ...
                case TdsEnums.SQLDATETIME2:
                    ...
                    WriteDateTime2(ValueTypeConverter<T, DateTime>.Convert(value), scale, actualLength, stateObj);
                    break;

                case TdsEnums.SQLDATETIMEOFFSET:
                    WriteDateTimeOffset(ValueTypeConverter<T, DateTimeOffset>.Convert(value), scale, actualLength, stateObj);
                    break;
        ...
        private byte[] SerializeUnencryptedSqlValue<T>(T value, MetaType type, int actualLength, int offset, byte normalizationVersion, TdsParserStateObject stateObj)
        {
            ...
            switch (type.NullableType)
            {
                case TdsEnums.SQLFLTN:
                    if (type.FixedLength == 4)
                    {
                        return SerializeFloat(ValueTypeConverter<T, SqlSingle>.Convert(value).Value);
                    }
                    else
                    {
                        Debug.Assert(type.FixedLength == 8, "Invalid length for SqlDouble type!");
                        return SerializeDouble(ValueTypeConverter<T, SqlDouble>.Convert(value).Value);
                    }
                ...
                case TdsEnums.SQLIMAGE:
                    {
                        ...
                        {
                            Buffer.BlockCopy(ValueTypeConverter<T, SqlBinary>.Convert(value).Value, offset, b, 0, actualLength);
                        }
                        else
                        {
                            Debug.Assert(value is SqlBytes);
                            Buffer.BlockCopy(ValueTypeConverter<T, SqlBytes>.Convert(value).Value, offset, b, 0, actualLength);
                        }
                ...
                case TdsEnums.SQLUNIQUEID:
                    {
                        byte[] b = ValueTypeConverter<T, SqlGuid>.Convert(value).ToByteArray();
                ...
                case TdsEnums.SQLBITN:
                    {
                        ...
                        // We normalize to allow conversion across data types. BIT is serialized into a BIGINT.
                        return SerializeLong(ValueTypeConverter<T, SqlBoolean>.Convert(value).Value == true ? 1 : 0, stateObj);
                    }

                case TdsEnums.SQLINTN:
                    // We normalize to allow conversion across data types. All data types below are serialized into a BIGINT.
                    if (type.FixedLength == 1)
                        return SerializeLong(ValueTypeConverter<T, SqlByte>.Convert(value).Value, stateObj);

                    if (type.FixedLength == 2)
                        return SerializeLong(ValueTypeConverter<T, SqlInt16>.Convert(value).Value, stateObj);

                    if (type.FixedLength == 4)
                        return SerializeLong(ValueTypeConverter<T, SqlInt32>.Convert(value).Value, stateObj);
                    else
                    {
                        Debug.Assert(type.FixedLength == 8, "invalid length for SqlIntN type:  " + type.FixedLength.ToString(CultureInfo.InvariantCulture));
                        return SerializeLong(ValueTypeConverter<T, SqlInt64>.Convert(value).Value, stateObj);
                    }
                ...
                case TdsEnums.SQLTEXT:
                    ...
                    {
                        Debug.Assert(value is SqlString);
                        return SerializeEncodingChar(ValueTypeConverter<T, SqlString>.Convert(value).Value, actualLength, offset, _defaultEncoding);
                    }
                ...
                case TdsEnums.SQLXMLTYPE:
                    ...
                    if (value is SqlChars)
                    {
                        return SerializeCharArray(ValueTypeConverter<T, SqlChars>.Convert(value).Value, actualLength, offset);
                    }
                    else
                    {
                        Debug.Assert(value is SqlString);
                        return SerializeString(ValueTypeConverter<T, SqlString>.Convert(value).Value, actualLength, offset);
                    }

                case TdsEnums.SQLNUMERICN:
                    Debug.Assert(type.FixedLength <= 17, "Decimal length cannot be greater than 17 bytes");
                    return SerializeSqlDecimal(ValueTypeConverter<T, SqlDecimal>.Convert(value), stateObj);
                ...
                case TdsEnums.SQLMONEYN:
                    {
                        return SerializeSqlMoney(ValueTypeConverter<T, SqlMoney>.Convert(value), type.FixedLength, stateObj);
                    }
