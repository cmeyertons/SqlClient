        ...
        internal static bool CoerceValueIfNeeded<T>(T value, MetaType destinationType, out object objValue, out bool coercedToDataFeed, bool allowStreaming = true)
        {
            ...
            try
            {
                ...
                if ((typeof(string) == destinationType.ClassType))
                {
                    ...
                    {
                        var xmlValue = ValueTypeConverter<T, SqlXml>.Convert(value);
                    ...
                    {
                        var charArrayValue = ValueTypeConverter<T, char[]>.Convert(value);
                        objValue = new string(charArrayValue);
                    }
                    else if (typeof(SqlChars) == currentType)
                    {
                        var sqlCharsValue = ValueTypeConverter<T, SqlChars>.Convert(value);
                ...
                {
                    objValue = decimal.Parse(ValueTypeConverter<T, string>.Convert(value), NumberStyles.Currency, (IFormatProvider)null);
                }
                ...
                {
                    objValue = TimeSpan.Parse(ValueTypeConverter<T, string>.Convert(value));
                }
                else if ((typeof(string) == currentType) && (SqlDbType.DateTimeOffset == destinationType.SqlDbType))
                {
                    objValue = DateTimeOffset.Parse(ValueTypeConverter<T, string>.Convert(value), (IFormatProvider)null);
                }
                else if ((typeof(DateTime) == currentType) && (SqlDbType.DateTimeOffset == destinationType.SqlDbType))
                {
                    objValue = new DateTimeOffset(ValueTypeConverter<T, DateTime>.Convert(value));
                }
